  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f4f6fb]">
      ...
      <div className="flex flex-1 min-h-0 w-full">
        <aside
          className={`fixed inset-y-0 left-0 z-40 w-64 bg-[#1a1c23] flex flex-col transition-transform duration-300 ${open ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:sticky lg:top-0 lg:self-start lg:h-screen lg:flex lg:z-10`}
        >
          <div className="px-6 py-5 border-b border-white/10">
            <Link to="/" className="flex items-center gap-2 min-w-0">
              <div className="w-8 h-8 shrink-0 rounded-lg flex items-center justify-center" style={{ background: ACCENT }}>
                <HeaderIcon className="w-4 h-4 text-white" />
              </div>
              <span className="text-white font-bold text-sm leading-tight truncate">DevelopmentWala.org</span>
            </Link>
          </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 w-full">
          <header className="bg-white border-b border-gray-200 px-4 py-2.5 flex items-center gap-3 sticky top-[72px] z-20 lg:hidden">
            ...
            <h1 className="text-base font-bold text-gray-900 truncate">{title || 'Dashboard'}</h1>
          </header>
